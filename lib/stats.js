...
// month before (to compare), and the last twelve months for the overview table.
...
export async function monthlyAnalytics({ month, tz = 0 }) {
  ...
  const months = Array.from({ length: 12 }, (_, i) => shiftMonth(current, -i)).map((key) => ({ month: key, ...tally(key) }));
  return {
    month: selected,
