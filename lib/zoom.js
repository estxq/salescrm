...
export async function createMeeting({ topic, startTime, durationMinutes = 45 }) {
  const meeting = await zoomFetch('/users/me/meetings', {
    ...
    body: JSON.stringify({
      ...
      // Deliberately NO `timezone` field: when Zoom is given one it reads start_time as a
      // local wall-clock time in that zone and ignores the trailing Z, which booked meetings
      // 8 hours early. Sent alone, the UTC instant is unambiguous.
...
export async function updateMeetingTime(meetingId, { startTime, durationMinutes }) {
  ...
  const after = await zoomFetch(`/meetings/${meetingId}`);
