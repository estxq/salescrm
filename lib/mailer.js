...
const ON_VERCEL = Boolean(process.env.VERCEL); // serverless: no persistent disk to save mock emails to
...
export async function sendEmail({ to, subject, html, trackingToken }) {
  ...
  if (!LIVE) {
    if (ON_VERCEL) {
      // No writable disk on serverless — the activity log already records
      // that this was sent; this is just a console trace for debugging.
