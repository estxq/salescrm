...
// Short-lived random value for an OAuth round trip (Zoom, Google), so the
// callback can prove it's the same browser that started it.
export function setOAuthState(req, res, state, provider = 'zoom') {
  setCookie(res, req, `sh_${provider}_state`, state, 600);
}
export function takeOAuthState(req, res, provider = 'zoom') {
  const state = readCookie(req, `sh_${provider}_state`);
  setCookie(res, req, `sh_${provider}_state`, '', 0);
  return state;
}
